...
import type { RecordOptions, RunFrame, RunManifest } from './types.js';
...
async function addFrame(
  ...
  frames.push(frame);
  if (runId) await appendFrameJsonl(runId, frame);
  return frame;
}

async function writeDiffIfPresent(
  runDir: string,
  name: string,
  diff?: string
...
export async function recordCommand(
  ...
  try {
    ...
    await addFrame(
      runId,
      frames,
      spawnError ? 'note' : 'exit',
      spawnError ? 'Command failed to start' : exitCode === 0 ? 'Run completed' : 'Run failed',
      spawnError ? spawnError.message : `exit code ${exitCode ?? 'unknown'}`,
      spawnError ? { error: spawnError.message } : { exitCode }
    );
