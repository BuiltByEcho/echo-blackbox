...
program
  ...
  .option(
    '--checkpoint <label>',
    'Add a manual checkpoint label before the command starts',
...
program
  ...
  .action(async (runValue: string, options) => {
...
program
  ...
  .argument('[run]', 'Run id or latest', 'latest')
  .option('--from <step>', 'Frame index to fork conceptually from', (value) =>
