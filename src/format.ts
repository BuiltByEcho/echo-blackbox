...
export function formatRunLine(run: RunManifest): string {
  const exit =
    typeof run.exitCode === 'number' ? ` exit=${run.exitCode}` : '';
  return `${run.id}  ${run.status.padEnd(7)}  ${run.name}  ${run.startedAt}${exit}`;
}

export function formatTimeline(run: RunManifest): string {
  const lines = [
  ...
  if (typeof run.exitCode === 'number') lines.push(`Exit: ${run.exitCode}`);
  if (run.git?.isRepo) {
    lines.push(
      `Git: ${run.git.branch || 'unknown'} @ ${(run.git.head || '').slice(0, 12)}`
    );
  }
  ...
  for (const frame of run.frames) {
    lines.push(
      `[${frame.index}] ${frame.kind.toUpperCase()} ${frame.title}${
...
export function formatMarkdown(run: RunManifest): string {
  const lines = [
    `# Echo Black Box Run: ${run.name}`,
    '',
    `- ID: \`${run.id}\``,
  ...
  for (const frame of run.frames) {
    ...
    if (frame.summary) lines.push(`- Summary: ${frame.summary}`);
    if (frame.data) {
      lines.push('', '```json');
